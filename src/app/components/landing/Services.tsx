    ...
    <Section id="layanan" className="bg-card">
      <Container>
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:mt-12 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
          {services.map((s, i) => (
            <SectionReveal key={s.id} delay={(i % 3) * 0.05} className="h-full">
              <div className="group flex h-full flex-col justify-between rounded-xl border border-border bg-background p-4 transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-lg sm:rounded-2xl sm:p-6">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground sm:mt-2 sm:text-sm">{s.description}</p>
                </div>
                <Button
                  asChild
                  variant="ghost"
